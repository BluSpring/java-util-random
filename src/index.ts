...
export default class Random {
    /** @type {bigint} */
    #seed: bigint = Random._seedUniquifier() ^ process.hrtime.bigint();
    /** @type {boolean} */
     ...
     */
    constructor(seed: number | bigint = Random._seedUniquifier() ^ process.hrtime.bigint()) {
        this.setSeed(seed);
    }
     ...
     */
    _checkIsNumber(n: number | bigint) {
        if (typeof n != 'number' && typeof n != 'bigint' || Number.isNaN(n)) {
...
/*function compareAndSet(oldVal: number, currentVal: number, newVal: number): boolean {
